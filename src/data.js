export const data = [
  // {
  //   id: 1,
  //   num: "001",
  //   name: "Bulbasaur",
  //   img: "http://www.serebii.net/pokemongo/pokemon/001.png",
  //   type: ["Grass", "Poison"],
  //   height: "0.71 m",
  //   weight: "6.9 kg",
  //   candy: "Bulbasaur Candy",
  //   candy_count: 25,
  //   egg: "2 km",
  //   spawn_chance: 0.69,
  //   avg_spawns: 69,
  //   spawn_time: "20:00",
  //   multipliers: [1.58],
  //   weaknesses: ["Fire", "Ice", "Flying", "Psychic"],
  //   next_evolution: [
  //     {
  //       num: "002",
  //       name: "Ivysaur",
  //     },
  //     {
  //       num: "003",
  //       name: "Venusaur",
  //     },
  //   ],
  // },
  {
    easy: [
      {
        id: 1,
        question: "What is the first book in the Bible?",
        answers: [
          {
            answer: "Genesis",
            correct: true,
          },
          {
            answer: "Exodus",
            correct: false,
          },
          {
            answer: "Revelation",
            correct: false,
          },
          {
            answer: "Micah",
            correct: false,
          },
        ],
      },
      {
        id: 2,
        question: "How many days did God take to create the world?",
        answers: [
          {
            answer: "6",
            correct: true,
          },
          {
            answer: "4",
            correct: false,
          },
          {
            answer: "7",
            correct: false,
          },
          {
            answer: "100",
            correct: false,
          },
        ],
      },
      // {
      //   id: 3,
      //   question: "Who was the first man?",
      //   answer: "Adam",
      //   wrongAnswers: ["Joel", "Abraham", "Judas"],
      // },
      // {
      //   id: 4,
      //   question: "Who was the first woman?",
      //   answer: "Eve",
      //   wrongAnswers: ["Ruth", "Esther", "Mary"],
      // },
      // {
      //   id: 5,
      //   question: "Where did Adam and Eve live at the beginning of the world?",
      //   answer: "Garden of Eden",
      //   wrongAnswers: ["Airbnb", "Heaven", "Israel"],
      // },
      // {
      //   id: 6,
      //   question: "Who were Adam and Eve's three sons listed in the Bible?",
      //   answer: "Cain, Abel, and Seth",
      //   wrongAnswers: [
      //     "Er, Onan, and Shelah",
      //     "Shadrach, Meshach, and Abednego",
      //     "Shem, Ham, and Japheth",
      //   ],
      // },
      // {
      //   id: 7,
      //   question: "Who did God tell to build an ark?",
      //   answer: "Noah",
      //   wrongAnswers: ["Moses", "Peter", "David"],
      // },
      // {
      //   id: 8,
      //   question:
      //     "How many days and nights did it rain when Noah was on the ark?",
      //   answer: "40",
      //   wrongAnswers: ["50", "30", "100"],
      // },
      // {
      //   id: 9,
      //   question:
      //     "What was God’s sign to Noah that he would never destroy the earth again?",
      //   answer: "A rainbow",
      //   wrongAnswers: ["An olive branch", "The dove", "A cloud"],
      // },
      // {
      //   id: 10,
      //   question: "How many brothers did Joseph have?",
      //   answer: "11",
      //   wrongAnswers: ["2", "5", "8"],
      // },
      // {
      //   id: 11,
      //   question:
      //     "What did Jacob give Joseph that sparked jealousy from his siblings?",
      //   answer: "A coat of many colors",
      //   wrongAnswers: ["A sheep", "New shoes", "A bowl of lentils"],
      // },
      // {
      //   id: 12,
      //   question: "How did Moses’ mother save him from the Egyptian soldiers?",
      //   answer: "Put him in a basket in the river",
      //   wrongAnswers: [
      //     "Hid him in a bush",
      //     "Rode away on a horse",
      //     "Put him in a tree",
      //   ],
      // },
      // {
      //   id: 13,
      //   question: "Through what did God speak to Moses in the desert?",
      //   answer: "A burning bush",
      //   wrongAnswers: ["A calf", "A sheep", "Walkie talkie"],
      // },
      // {
      //   id: 14,
      //   question: "How many plagues did God send on Egypt?",
      //   answer: "10",
      //   wrongAnswers: ["20", "15", "1"],
      // },
      // {
      //   id: 15,
      //   question: "What did God do through Moses to save the slaves?",
      //   answer: "Parted the Red Sea",
      //   wrongAnswers: [
      //     "Blinded the Soldiers",
      //     "Brought a huge boat",
      //     "Hid them in a fog",
      //   ],
      // },
      // {
      //   id: 16,
      //   question: "Where did God give Moses the Ten Commandments?",
      //   answer: "Mt. Sinai",
      //   wrongAnswers: ["The Garden of Eden", "Mt. Moriah", "Mt. Gaash"],
      // },
      // {
      //   id: 17,
      //   question: "What golden image did the Israelites make at Mt. Sinai?",
      //   answer: "A golden calf",
      //   wrongAnswers: ["A golden sheep", "A golden duck", "A golden donkey"],
      // },
      // {
      //   id: 18,
      //   question: "Who was the first king of Israel?",
      //   answer: "Saul",
      //   wrongAnswers: ["David", "Solomon", "Zedekiah"],
      // },
      // {
      //   id: 19,
      //   question:
      //     "When Daniel prayed to God after it was not allowed, where was he thrown into?",
      //   answer: "The lion’s den",
      //   wrongAnswers: [
      //     "The spider tent",
      //     "The serpent pit",
      //     "The scorpion cave",
      //   ],
      // },
      // {
      //   id: 20,
      //   question: "Name the city where Jesus was born.",
      //   answer: "Bethlehem",
      //   wrongAnswers: ["Jerusalem", "Jericho", "Nazareth"],
      // },
      // {
      //   id: 21,
      //   question: "Who baptized Jesus?",
      //   answer: "John the Baptist",
      //   wrongAnswers: ["Peter", "John", "Simon"],
      // },
      // {
      //   id: 22,
      //   question: "Before Jesus started preaching, what was his job?",
      //   answer: "Carpenter",
      //   wrongAnswers: ["Farmer", "Tax Collector", "Telemarketer"],
      // },
      // {
      //   id: 23,
      //   question: "Name the hometown of Jesus",
      //   answer: "Nazareth",
      //   wrongAnswers: ["Bethlehem", "Jerusalem", "Jericho"],
      // },
      // {
      //   id: 24,
      //   question: "True or False: Jesus turned water into Pepsi.",
      //   answer: "False, Jesus turned water into wine",
      //   wrongAnswers: ["True"],
      // },
      // {
      //   id: 25,
      //   question: "Name the place where Jesus walked on water?",
      //   answer: "Sea of Galilee",
      //   wrongAnswers: ["Red Sea", "The Great Sea", "The Mediterranean Sea"],
      // },
      // {
      //   id: 26,
      //   question: "How many disciples did Jesus choose?",
      //   answer: "12",
      //   wrongAnswers: ["11", "10", "5"],
      // },
      // {
      //   id: 27,
      //   question:
      //     "How much bread and fish did Jesus use to feed more than 5,000 people?",
      //   answer: "Five loaves of bread—and two fish",
      //   wrongAnswers: [
      //     "Two loaves of bread—and two fish",
      //     "Eight loaves of bread—and three fish",
      //     "Five loaves of bread—and three fish",
      //   ],
      // },
      // {
      //   id: 28,
      //   question: "What did Jesus do at the Last Supper to his disciples?",
      //   answer: "Washed their feet",
      //   wrongAnswers: [
      //     "Made them a meal",
      //     "Told a story",
      //     "Asked them all a question",
      //   ],
      // },
      // {
      //   id: 29,
      //   question:
      //     "What did the woman pour on Jesus’ feet at the home of Simon the Leper?",
      //   answer: "Jar of expensive perfume",
      //   wrongAnswers: ["Jar of water", "Jar of oil", "Jar of honey"],
      // },
      // {
      //   id: 30,
      //   question: "Name the disciple who betrayed Jesus.",
      //   answer: "Judas Iscariot",
      //   wrongAnswers: ["Simon the Zealot", "Matthew", "John"],
      // },
      // {
      //   id: 31,
      //   question: "Which disciple denied Jesus three times?",
      //   answer: "Peter",
      //   wrongAnswers: ["Judas Iscariot", "Bartholomew", "Thomas"],
      // },
      // {
      //   id: 32,
      //   question: "How many books have the name John in them?",
      //   answer: "Four",
      //   wrongAnswers: ["One", "Three", "Two"],
      // },
      // {
      //   id: 33,
      //   question: "Name the person who received the 10 Commandments from God?",
      //   answer: "Moses",
      //   wrongAnswers: ["Abraham", "Noah", "Ezekiel"],
      // },
      // {
      //   id: 34,
      //   question: "What is the 1st commandment?",
      //   answer: "You shall have no other gods before me",
      //   wrongAnswers: [
      //     "Honor your father and mother",
      //     "You shall not steal",
      //     "Love your neighbor as yourself",
      //   ],
      // },
      // {
      //   id: 35,
      //   question: "What is the 5th commandment?",
      //   answer: "Honor your father and mother",
      //   wrongAnswers: [
      //     "You shall not steal",
      //     "You shall have no other gods before me",
      //     "Love your neighbor as yourself",
      //   ],
      // },
      // {
      //   id: 36,
      //   question: "What is the 8th commandment?",
      //   answer: "You shall not steal",
      //   wrongAnswers: [
      //     "You shall have no other gods before me",
      //     "Honor your father and mother",
      //     "Love your neighbor as yourself",
      //   ],
      // },
      // {
      //   id: 37,
      //   question: "What does Jesus say is the second greatest commandment?",
      //   answer: "Love your neighbor as yourself",
      //   wrongAnswers: [
      //     "You shall not steal",
      //     "You shall have no other gods before me",
      //     "Honor your father and mother",
      //   ],
      // },
      // {
      //   id: 38,
      //   question: "What is the last book in the Bible?",
      //   answer: "Revelation",
      //   wrongAnswers: ["Jude", "Genesis", "Malachi"],
      // },
      // {
      //   id: 39,
      //   question: "What four books tell about Jesus' life on Earth?",
      //   answer: "Matthew, Mark, Luke, John",
      //   wrongAnswers: [
      //     "Genesis, Exodus, Leviticus, Numbers",
      //     "Daniel, Hosea, Joel, Amos",
      //     "Job, Psalms, Proverbs, Ecclesiastes",
      //   ],
      // },
      // {
      //   id: 40,
      //   question: "Which book did David write most of?",
      //   answer: "Psalms",
      //   wrongAnswers: ["Haggai", "Genesis", "Jeremiah"],
      // },
      // {
      //   id: 41,
      //   question: "What were men trying to do at the Tower of Babel?",
      //   answer: "Build a tower to reach Heaven",
      //   wrongAnswers: [
      //     "Build a tower to get better cell service",
      //     "Build a tower for fun",
      //     "Build the smallest tower",
      //   ],
      // },
      // {
      //   id: 42,
      //   question: "How did the 12 brothers get rid of Joseph?",
      //   answer: "Sold him to slave traders",
      //   wrongAnswers: [
      //     "Tied him on a horse",
      //     "Locked him out of the house",
      //     "Bought him a new house",
      //   ],
      // },
      // {
      //   id: 43,
      //   question: "Who bought Joseph?",
      //   answer: "Potiphar",
      //   wrongAnswers: ["Potiphar's wife", "The cup-bearer", "King Pharaoh"],
      // },
      // {
      //   id: 44,
      //   question: "Who got Joseph thrown into prison by lying about him?",
      //   answer: "Potiphar’s wife",
      //   wrongAnswers: ["The cup-bearer", "King Pharaoh", "Potiphar"],
      // },
      // {
      //   id: 45,
      //   question:
      //     "What catastrophe did Joseph predict by interpreting the Pharaoh’s dream?",
      //   answer: "A severe, seven-year famine",
      //   wrongAnswers: [
      //     "There would a great flood",
      //     "There would be a drought",
      //     "There would be a plague of locust",
      //   ],
      // },
      // {
      //   id: 46,
      //   question: "What was the giant’s name that David killed?",
      //   answer: "Goliath",
      //   wrongAnswers: ["Schwarzenegger", "Og", "Bigfoot"],
      // },
      // {
      //   id: 47,
      //   question: "How did David kill Goliath?",
      //   answer: "A slingshot and a stone",
      //   wrongAnswers: ["A sword", "A bow and arrow", "He tripped him"],
      // },
      // {
      //   id: 48,
      //   question: "Who was Jesus' most famous cousin?",
      //   answer: "John the Baptist",
      //   wrongAnswers: ["Elizabeth", "Abraham", "Matthew"],
      // },
      // {
      //   id: 49,
      //   question: "What were the names of Jesus’s brothers?",
      //   answer: "James, Joseph, Simon and Judas",
      //   wrongAnswers: [
      //     "Matthew, John, Luke and Peter",
      //     "Cain, Abel, Jacob and Esau",
      //     "Simon, Levi, Zebulun and Issachar",
      //   ],
      // },
      // {
      //   id: 50,
      //   question: "How many lepers did Jesus heal?",
      //   answer: "10",
      //   wrongAnswers: ["5", "20", "1"],
      // },
    ],
    medium: [
      {
        id: 1,
        question: "What is the first book in the Bible?",
        answers: [
          {
            answer: "Genesis",
            correct: true,
          },
          {
            answer: "Exodus",
            correct: false,
          },
          {
            answer: "Revelation",
            correct: false,
          },
          {
            answer: "Micah",
            correct: false,
          },
        ],
      },
      {
        id: 2,
        question: "How many days did God take to create the world?",
        answers: [
          {
            answer: "6",
            correct: true,
          },
          {
            answer: "4",
            correct: false,
          },
          {
            answer: "7",
            correct: false,
          },
          {
            answer: "100",
            correct: false,
          },
        ],
      },
    ],
    hard: [
      {
        id: 1,
        question: "What is the first book in the Bible?",
        answers: [
          {
            answer: "Genesis",
            correct: true,
          },
          {
            answer: "Exodus",
            correct: false,
          },
          {
            answer: "Revelation",
            correct: false,
          },
          {
            answer: "Micah",
            correct: false,
          },
        ],
      },
      {
        id: 2,
        question: "How many days did God take to create the world?",
        answers: [
          {
            answer: "6",
            correct: true,
          },
          {
            answer: "4",
            correct: false,
          },
          {
            answer: "7",
            correct: false,
          },
          {
            answer: "100",
            correct: false,
          },
        ],
      },
    ],
  },
];
